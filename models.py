...
class MLPPlanner(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        Called with model(track_left=..., track_right=...)
...
class TransformerPlanner(nn.Module):
    def __init__(
        ...
        self.blocks = nn.Sequential(
            TransformerBlock(embed_dim=d_model),
            TransformerBlock(embed_dim=d_model)
        )
...
class ViTPlanner(torch.nn.Module):
    def __init__(
        ...
        Add'l arguments can be added to the constructor such as:
        ...
        self.patch_embedding = PatchEmbedding(h=96, w=128, patch_size=patch_size, in_channels=3, embed_dim=embed_dim)

        self.positional_embeddings = nn.Parameter(torch.zeros(1, self.patch_embedding.num_patches, 64))

        self.transformer_blocks = nn.Sequential(
            TransformerBlock(embed_dim=64),
            TransformerBlock(embed_dim=64),
            TransformerBlock(embed_dim=64),
        )

        self.output_head = nn.Linear(64, n_waypoints * 2)
    ...
    def forward(self, image: torch.Tensor, **kwargs) -> torch.Tensor:
        ...
        out = out.view(image.size(0), self.n_waypoints, 2)
        return out
...
def load_model(
    ...
    **model_kwargs,
    ...
    if with_weights:
        model_path = MAIN_DIR / f"{model_name}.th"
...
def save_model(model: torch.nn.Module) -> str:
    ...
    output_path = MAIN_DIR / f"{model_name}.th"
